    ...
    app.post("/add-test-data", async (req, res) => {
      try {
        const testData = [
          ...
          { name: "Dental Cleaning", slots: ["10:30 AM", "12:00 PM"] },
        ];
        const result = await appointmentOptionCollection.insertMany(testData);
        res.send({ message: "Test data added", result });
      } catch (err) {
        console.error("Error adding test data:", err);
        res.status(500).send({ error: "Failed to add test data" });
      }
    });
